...
{
    className_ = std::string("NeuroTreeCluster");
...
ProcStatPointer NeuroTreeCluster::Update()
{
    if(!m_Input ){
    ...
    std::shared_ptr<NeuronPopulation> tmpSeperateTree = std::dynamic_pointer_cast<NeuronPopulation>(m_Source);

    VectorVec3d somaList;
    Vec3d tmp;

    //break curve connection  //2017-9-10 maybe bug the connect node will disappear
    ...
    //fill the gap after break
    for (size_t i = 0; i < tmpConnect->size(); ++i){
        const Mat2i &curConInfo = tmpConnect->GetConnect()[i];
        if (curConInfo(0,0) > 0) {
            VectorVec5d& curCurve = rawDendList[i];
            Vec3d headPt = curCurve.front().block(0, 0, 3, 1);
            VectorVec5d& conCurve = rawDendList[curConInfo(0, 0)-1];
            double tmpDist, minDist = 1000000.0; int minId = -1;
            for (size_t j = 0; j < conCurve.size(); ++j) {
                tmpDist = (conCurve[j].block(0, 0, 3, 1) - headPt).norm() < minDist;
                if ( tmpDist <minDist ) {
                    minDist = tmpDist;
                    minId = j;
                }
            ...
            if (minDist > 0.1) 
                conCurve[minId].block(0,0,3,1) = headPt;
        }
        if (curConInfo(0, 1) > 0) {
            VectorVec5d& curCurve = rawDendList[i];
            Vec3d tailPt = curCurve.back().block(0, 0, 3, 1);
            VectorVec5d& conCurve = rawDendList[curConInfo(0, 1) - 1]; double tmpDist, minDist = 1000000.0; int minId = -1;
            for (size_t j = 0; j < conCurve.size(); ++j) {
                tmpDist = (conCurve[j].block(0, 0, 3, 1) - tailPt).norm() < minDist;
                if (tmpDist < minDist) {
                    minDist = tmpDist;
                    minId = j;
                }
            ...
            if (minDist > 0.1)
                conCurve[minId].block(0, 0, 3, 1) = tailPt;
        }
    }
    ...
    for(int final = 0; final < int(dendCurvesCluster.size());  ++final){
        ...
        if(singleCellLabel.empty()) {
            if (tmpSoma->size() > 0) continue;//do not save trees without soma if there are other soma
            printf("   cell = 0\n");
        ...
        if(singleCellLabel.size() == 1lu){
            ...
            for(int i = 0; i < (pathGraph1XLen - assignedDendNum); ++i){
                if(0==newAssignedIDList[i + assignedDendNum]){
                    ...
                    //-----------------------2014-6-30---------------------------//
                    SearchTreeFromSomaModify(tmpPathList3, i + assignedDendNum, 200,
            ...
            for(std::vector<int>::size_type i = 0; i < somaTreeInClustre.size();++i){
                ...
                std::vector<int> resultTreeInClustre = currentSomaTree;
...
#pragma omp critical
                {
                    ...
                    typeList.push_back(2);
                    /*int oldSz = int(availableTreeInClustre.size());//annotation
    ...
    std::vector<NeuronTreePointer>& neuronTreeList = tmpSeperateTree->m_pop;
    neuronTreeList.resize(availableTreeInClustre.size());
    std::for_each(neuronTreeList.begin(), neuronTreeList.end(), [](NeuronTreePointer& arg){if (!arg)
        arg = std::make_shared<NeuronTree>();
    });
    for (size_t i = 0; i < availableTreeInClustre.size(); ++i){
        std::vector<Line5d >& curves = neuronTreeList[i]->m_curveList;
        tree<LineID> &topo = neuronTreeList[i]->m_Topo;//no head
        std::vector<int>& tmpID = availableTreeInClustre[i];
        if (curves.size() == 1llu && curves[0].size() == 1llu) {
            curves.push_back(newDendList[tmpID[0] - 1]);
            topo.insert(topo.begin(), LineID(0));
        }
        else{
            for (size_t j = 0; j < tmpID.size(); ++j){
                if (newDendList[tmpID[j] - 1].size() > 2lu) {//filter soma
                    curves.push_back(newDendList[tmpID[j] - 1]);
                    topo.insert(topo.begin(), LineID(j));
                }
            }
        }
        
        if (tmpSoma->size() <1) 
            neuronTreeList[i]->m_type = 0;//debug
        else
            neuronTreeList[i]->m_type = 2;//soma tree
    }
    neuronTreeList.erase(std::remove_if(neuronTreeList.begin(), neuronTreeList.end(), [](NeuronTreePointer &arg){return arg->m_curveList.empty(); }), neuronTreeList.end());
...
ConstIDataPointer NeuroTreeCluster::GetOutput()
{
    if(!m_Source)
#ifdef _WIN32
...
IDataPointer NeuroTreeCluster::ReleaseData()
{
...
void NeuroTreeCluster::SetInputCurve(ConstIDataPointer arg)
{
    m_Curve = arg;
}

void NeuroTreeCluster::SetInputSoma(ConstIDataPointer arg)
{
    m_Soma = arg;
}

void NeuroTreeCluster::SetInputOrigImage(ConstIDataPointer arg)
{
    m_OrigImg =arg;
}
...
//    generate_n(back_inserter(idexxS), blocksum, GenArray<double>(1.0, 1.0));
    for(std::vector<double>::size_type i = 1; i <= raySlice.size(); ++i){
        lineSegLength.push_back(double(i));
    }
        ...
        if(kk > 0){
            ...
            for(int jj = 0; jj < kk; ++jj){
                ...
                for(VectorVec4d::size_type k = 0; k < aNewTreeLevel.size(); ++k){
                    allNewTreeLevelList.push_back(NGUtility::Round(aNewTreeLevel[k](0)));
                }
                for(VectorVec4d::size_type k = 0; k < aNewTreeLevel.size(); ++k){
                    allNewTreeLevelList.push_back(NGUtility::Round(aNewTreeLevel[k](1)));
                }
            }
                                   ...
                                   std::vector<int> &resultSomaTree)
{
    ...
    for(int i = 2; i < depth; ++i){
        ...
        for(VectorVec4d::size_type k = 0; k < currentTreeLevel.size();++k){
            tmp.push_back(NGUtility::Round(currentTreeLevel[k](0)));
        }
        ...
        for(int jj = 0; jj < ns - 1; ++jj){
            ...
            if(wrongConnectID.size() == 1){
                ...
                for(std::vector<int>::size_type kk = 0; kk < wrongConnectID.size();
                    ++kk){
                    tmpCurrentTreeLevel.push_back(NGUtility::Round(currentTreeLevel[
            ...
            for(int ijj = 0; ijj < nlls; ++ijj){
                double m = double(dendSomaList[currentSomaTree[NGUtility::Round(currentTreeLevel[idexx[jj] + ijj + 1](1))] - 1].size());
                curveLen += m;
            ...
            if (angleLenRatio < -0.3){//
                ...
                for(int k = idexx[jj]+1; k <= idexx[jj+1];++k ){
                    mm[k] = 0;
                    tmpCurrentTreeLevel.push_back(NGUtility::Round(currentTreeLevel[k](1)));
                }
                std::vector<std::vector<int> > levelNodeFlagCp;
                GetTreeLevelFlagList(levelNodeFlag, i, tmpCurrentTreeLevel, treeLevel,
                                     levelNodeFlagCp);
    ...
    for(int i = 2; i < depth; ++i){
        ...
        for(int jj = 0; jj < currentTreeLevelNum; ++jj){
            const VectorVec5d& globalOneLevelIDList = dendSomaList[currentSomaTree[NGUtility::Round(currentTreeLevel[jj](1))] - 1];
        ...
        for(int ijj = 0; ijj < currentTreeLevelNum; ++ijj){
            if(medianRadiusList[ijj] > thre){//
                ...
                GetTreeLevelFlagList(levelNodeFlag, i,
                    std::vector<int>(1, NGUtility::Round(currentTreeLevel[ijj](1))),
                                     treeLevel,
                                     levelNodeFlagCp);
                levelNodeFlag.swap(levelNodeFlagCp);
            }
    ...
    for(int i = 0; i < depth; ++i){
        ...
        for(std::vector<int>::size_type j = 0; j < idexxm.size();++j){
            resultSomaTree.push_back(NGUtility::Round(dataK[idexxm[j]](1)));
        }
    }
                                             ...
                                             std::vector<std::vector<int> > &newLevelNodeFlag)
{
    ...
    for(int i = currentLevelID + 1; i < nxx; ++i){
        ...
        for(VectorVec4d::size_type j = 0; j < currentLevelDepth.size();++j){
            currentLevelDepthID.push_back(NGUtility::Round(currentLevelDepth[j](0)));
        }
