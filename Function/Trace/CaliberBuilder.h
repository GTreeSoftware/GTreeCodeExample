...
public:
	...
	void SetLinePoints(const std::vector<Line5d>& arg);
	...
	void GetRadius(std::vector<std::vector<double>>& r){ r = RRR; };
	void GetCaliberSignal(std::vector<std::vector<int>>& s){ s = CaliberSignal; };
...
protected:
	...
	std::vector<std::vector<double>> RRR;
	std::vector<std::vector<int>> CaliberSignal;
	...
	void NeuritesShapeReconModifyTotal_Weak(CellVec3d &CurveSet, VectorVec2d& ConnectSet, SVolume &OrigImag, CellVec3d &ShapePointSet);
	void SampleCylinderDataSetsExtraModify(VectorVec3d &Curves, SVolume & OrigIma, int ScaleSubImage, float Pixelsize, VectorMatXd &SampleDataset, MatXd &DirecVecs);
	void SBGCSTotalTT(const MatXd& ImageF, MatXd &ImagPrio, double Lamda0, double Threv, double LamdaPrio, double Threv_, MatXd & USeg, MatXd &U, Vec2d & c1_c2);
	void SBGCSTotalTT_new(const MatXd& ImageF, MatXd &ImagPrio, double Lamda0, double Threv, double LamdaPrio, double Threv_, MatXd & USeg, MatXd &U, Vec2d & c1_c2, MatXd &F);
	...
	//void NeuritesShapeReconModifyTotal(CellVec3d &CurveSet, VectorVec2d& ConnectSet, SVolume &OrigImag, CellVec3d &ShapePointSet);
	void NeuritesShapeReconModify(CellVec3d & CurveSet, SVolume & OrigImage, int SampleImgsize, CellVec3d &ShapePointSet);
	...
	void SegmentpointModify(VectorMatXd &Segmentdata, MatXd &DircVecs, SVolume &OrigImag, VectorVec2d &ThresMatrix, std::vector<int> MatrixFlag, CellVec7d &dataPointSet);
	void Segmentpoint(VectorMatXd &Segmentdata, MatXd &DircVecs, SVolume &OrigImag, VectorVec2d &ThresMatrix, CellVec7d &dataPointSet);
	void LabelImage(MatXd&LabelImag);
	void LabelImage(MatXd&LabelImag, MatXd&L);
	...
	void SegmentPointSceening(CellVec7d &dataPointSet, SVolume &ImagLabel, SVolume &OrigImage, int CurveLabel, VectorVec3d & dataSet, VecXd& Labelvec, std::vector<VectorVec3d> &dpS, std::vector<VectorVec3d> &dS);
	...
	void FilleddataPointModify_Cell(CellVec7d &dataPointSet, SVolume & OrigImag, VectorVec3d &dataPointS);
	void CurvesDircs(VectorVec3d &Curves, MatXd&DirecVecs);
	void ComMassPt(MatXd& MMkk, MatXd &USeg, Vec2d &CenterPt, double thre, Vec2d &c1_c2, int &tag);
	Vec2d massCenterCom(VectorVec2d &matCord, MatXd &areaLabel, MatXd &MMkk);
