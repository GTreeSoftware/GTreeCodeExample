...
ProcStatPointer CrossFiberFilter::Update()
{
    ...
    //build tree topology
    //KPTREE::SetTreeRoot(rawTraceLine, topo_, soma_, 1.0, false);//because the trace methods decide that the curve is traced outward, so T style is impossible.
        ...
        for (size_t i = 0; i < rawTraceLine.size(); ++i) {// line
            for (size_t j = 0; j < curConnectPtSet.size(); ++j) {//connect point
                ...
                if ((pt.block(0, 0, 3, 1) - curConnectPtSet[j]).norm() < 1.0) {
                    ...
                    curIt->id += prefix_*(k+1);//label represent the connect point cluster num !!!!!
