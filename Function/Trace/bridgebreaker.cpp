...
ProcStatPointer BridgeBreaker::Update()
{
    if(!m_Input  ){
    ...
    std::shared_ptr<TreeConnect> tmp_Source = std::dynamic_pointer_cast<TreeConnect>(m_Source);
#endif
    //WriteVectorVectorVec5d("X:/TDI11302011_mostd/nima1.swc", (*tmpTree).GetCurve());
    //system("pause");
...
ConstIDataPointer BridgeBreaker::GetOutput()
{
    if(!m_Source)
        m_Source = std::shared_ptr<TreeConnect>(new TreeConnect(this));
    return m_Source;
...
IDataPointer BridgeBreaker::ReleaseData()
{
    ...
    for(int i = 0; i < nxx; ++i){
        ...
        if(currentDendConInfo(0,0) != 0 && currentDendConInfo(0,1) != 0 ){
            ...
            if(currentDendConInfo(0,0) > 0){
                ...
                connectFlag = 0.5 * Vec2d( NGUtility::sign(std::abs<double>(double(conjHeadDendConInfo(0,0) ) )),
                    NGUtility::sign(std::abs<double>(double(conjHeadDendConInfo(0, 1)))));
...
#pragma omp critical
            ...
            if(currentDendConInfo(0,1) > 0){
                ...
                connectFlag = 0.5 * Vec2d(NGUtility::sign(std::abs<double>(double(conjTailDendConInfo(0, 0)))),
                    NGUtility::sign(std::abs<double>(double(conjTailDendConInfo(0, 1)))));
                                 ...
                                 int &knotFlag)
{
    ...
    if(directionFlag == 0){
        ...
        int idexx =int( std::distance(distList.begin(), minItem));
...
//    }
//
//    std::vector<double> tmp_diff;//original gradient
//    //diff_vector<double>(one_ray_wet, tmp_diff);//interp1
//    tmp_diff.clear();
//    for (std::vector<double>::size_type i = 0; i < one_ray_wet.size() - 1; ++i){
//        tmp_diff.push_back(one_ray_wet[i + 1] - one_ray_wet[i]);
...
//                idMax = std::min<int>(4, int(currentHeadIdx.size()) - 1);//5-1=4
//                for(int k = 0; k <= idMax; ++k){
//                    vd3 += conjDendCurve[currentHeadIdx[k] ](4);//2015-8-13
//                }
//                vd3 /= idMax + 1;
//
//                double vdMax = vd1 > vd2 ? (vd1 > vd3 ? vd1 : vd3) : (vd2 > vd3 ? vd2 : vd3);
