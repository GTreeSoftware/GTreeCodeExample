...
#include "traceutil.h"
#include "../NGUtility.h"
#include "./SparseTrace/SVMTraceFilter.h"
...
{
    className_ = std::string("TraceFilter");
...
ProcStatPointer TraceFilter::Update()
{
...
#endif
    //true method
    //LOG(INFO)<<"TraceCurvesAndConInfo";
    ...
    LOG(ERROR) << int(end - beg) <<" ms eclipsed in raw trace. ";
#else
    gettimeofday(&end1, 0);
    double timeuse=1000000*(end1.tv_sec-start1.tv_sec)+end1.tv_usec-start1.tv_usec;
    timeuse/=1000000;
    printf("%lf s eclipsed in raw trace\n", timeuse);
    LOG(INFO) << timeuse << " s eclipsed in raw trace";
#endif
    MAKEPROCESSSTATUS(resSta, true, className_, "");
    return resSta;
}

INEURONPROCESSOBJECT_GETOUTPUT_IMPLE(TraceFilter, TreeConnect)

IDataPointer TraceFilter::GetConnect()
{
    return rawDendConInfoPointer;
}

INEURONPROCESSOBJECT_RELEASEDATA_IMPLE(TraceFilter)

void TraceFilter::SetInputBack(ConstIDataPointer p)
{
    m_Back = p;
}

void TraceFilter::SetInputBin(ConstIDataPointer p)
{
    m_Bin = p;
}

void TraceFilter::SetSoma(ConstIDataPointer p)
{
    m_Soma = p;
}

IDataPointer &TraceFilter::GetDendConInfo()
{
    return rawDendConInfoPointer;
}
...
#pragma omp parallel for
    ...
    for (VectorVec3d::size_type i = 0; i < somaNum; ++i){//TODO
        ...
        LOG(INFO) << "cell tracing :" << int(i) + 1 << " of " << int(somaNum);
        ...
        if(somaInitDir.col(0).array().abs().sum() > 0.0){
            ...
            if(!thickCurve.empty()){
                TraceCurvesFromSoma(origImg, backImg, resultIndexImg,thickCurve,  int(i)+1,somaInitDir,
                                    somaList,
                                    resultCurve, resultCurConInfo);
            } else{
                resultCurve.clear();
            }
        }else{
    ...
    printf("\n");
    int seedSeries = int(traceSeed.size() - somaNum);//2016-12-21
    seedSeries = seedSeries > 10000 ? int(0.7 * seedSeries + 0.5) : seedSeries;
    int uppersz = int(somaNum) + seedSeries;
    for (int i = int(somaNum); i < uppersz; ++i){
        printf("                \r");
        printf("seed tracing : %d of %d\r", int(i - somaNum + 1), seedSeries);
        LOG(INFO) << "seed tracing :" << int(i - somaNum + 1)<<" of " <<seedSeries;
        Vec3d initialPoint(traceSeed[i](0), traceSeed[i](1), traceSeed[i](2));
    ...
    //check connect 
    int id1, id2; bool errorNima = false;
    Vec3d curPt;
    for (size_t k = 0; k < rawDendConInfo.size(); ++k) {
        id1 = rawDendConInfo[k](0, 0);
        id2 = rawDendConInfo[k](0, 1);
        errorNima = false;
        if (id1 > 0) {
            errorNima = true;
            curPt << rawDendList[k].front()(0), rawDendList[k].front()(1), rawDendList[k].front()(2);
            for (auto &it : rawDendList[id1-1]) {
                if ( (curPt-it.block(0,0,3,1)).norm() < 1.0) {
                    errorNima = false;
                    break;
                }
            }
            if (errorNima) {
                NG_ERROR_MESSAGE("trace filter error");
                system("pause");
            }
        }
        if (id2 > 0) {
            errorNima = true;
            curPt << rawDendList[k].back()(0), rawDendList[k].back()(1), rawDendList[k].back()(2);
            for (auto &it : rawDendList[id2-1]) {
                if ((curPt - it.block(0, 0, 3, 1)).norm() < 1.0) {
                    errorNima = false;
                    break;
                }
    ...
    //NGUtility::WriteVectorVectorVec5d("X:/TDIE1301a008/nima.swc", rawDendList);
    //system("pause");
}
                                            ...
                                            Vec3d &firDir, Vec3d &secDir)
{
    ...
    xExtractRegion = yExtractRegion = 10.0;//2016-3-8
    zExtractRegion = 10.0;
    
    int xMin = std::max(NGUtility::Round(curSeedNode(0) - xExtractRegion ), 0);
    int xMax = std::min(NGUtility::Round(curSeedNode(0) + xExtractRegion), vx - 1);
    int yMin = std::max(NGUtility::Round(curSeedNode(1) - yExtractRegion ), 0);
    int yMax = std::min(NGUtility::Round(curSeedNode(1) + yExtractRegion ), vy - 1);
    int zMin = std::max(NGUtility::Round(curSeedNode(2) - zExtractRegion ), 0);
    int zMax = std::min(NGUtility::Round(curSeedNode(2) + zExtractRegion ), vz - 1);
        ...
        for (int i = 0; i < 36; ++i){
            for ( int j = 0; j < 18; ++j){
                ...
                if (angleDiff < angleThrev1) //if jd < -0.85
                {
                    ...
                    rayNode.push_back(curCenter + 3 * polarPosition);//2016-3-8
                    rayNode.push_back(curCenter + 4.5 * polarPosition);
                    rayNode.push_back(curCenter + 6 * polarPosition);
                    rayNode.push_back(curCenter + 7.5 * polarPosition);
        ...
        for (int i = 0; i < 36; ++i){
            for ( int j = 0; j < 18; ++j){
                ...
                if (angleDiff > angleThrev2){ //if jd > -0.85
                    ...
                    rayNode.push_back(curCenter + 3 * ds1);//2016-3-8
                    rayNode.push_back(curCenter + 4.5 * ds1);
                    rayNode.push_back(curCenter + 6 * ds1);
                    rayNode.push_back(curCenter + 7.5 * ds1);
                    rayNode.push_back(curCenter + 9 * ds1);
                    std::vector<double> rayNodeWet;
                    WeighRayValue(rayNode, locOrigImg, rayNodeWet);
    ...
    for (int i = 0; i < 3; ++i){//2014-4-21
        ...
        int minX = std::max(std::min(NGUtility::Round(rayNode(0) - 3.0), vx - 1), 0);//2016-3-8
        int maxX = std::min(std::max(NGUtility::Round(rayNode[0] + 3.0), 0), vx - 1);
        int minY = std::max(std::min(NGUtility::Round(rayNode[1] - 3.0), vy - 1), 0);
        int maxY = std::min(std::max(NGUtility::Round(rayNode[1] + 3.0), 0), vy - 1);
        int minZ = std::max(std::min(NGUtility::Round(rayNode[2] - 3.0), vz - 1), 0);
        int maxZ = std::min(std::max(NGUtility::Round(rayNode[2] + 3.0), 0), vz - 1);
    ...
    for (int i = 1; i < 3; ++i){
        ...
        int minX = std::max(std::min(int(rayNode[0] - 3.0 + 0.5), vx - 1), 0);//2016-3-8
        int maxX = std::min(std::max(int(rayNode[0] + 3.0 + 0.5), 0), vx - 1);
        int minY = std::max(std::min(int(rayNode[1] - 3.0 + 0.5), vy - 1), 0);
        int maxY = std::min(std::max(int(rayNode[1] + 3.0 + 0.5), 0), vy - 1);
        int minZ = std::max(std::min(int(rayNode[2] - 3.0 + 0.5), vz - 1), 0);
        int maxZ = std::min(std::max(int(rayNode[2] + 3.0 + 0.5), 0), vz - 1);
        //Volumn localVol(maxX - minX + 1, maxY - minY + 1, maxZ - minZ + 1);
        int lx = maxX - minX + 1;
        int ly = maxY - minY + 1;
        int lz = maxZ - minZ + 1;
                                     ...
                                     Vec3d &firDir, Vec3d &secDir)
{
    ...
    for (int i = 0; i < 5; ++i){//2015-8-13
        ...
        int minX = std::max(std::min(NGUtility::Round(rayNode(0) - 3.0), vx - 1), 0);
        int maxX = std::min(std::max(NGUtility::Round(rayNode[0] + 3.0), 0), vx - 1);
        int minY = std::max(std::min(NGUtility::Round(rayNode[1] - 3.0), vy - 1), 0);
        int maxY = std::min(std::max(NGUtility::Round(rayNode[1] + 3.0), 0), vy - 1);
        int minZ = std::max(std::min(NGUtility::Round(rayNode[2] - 2.0), vz - 1), 0);
        int maxZ = std::min(std::max(NGUtility::Round(rayNode[2] + 2.0), 0), vz - 1);
        //Volumn localVol(maxX - minX + 1, maxY - minY + 1, maxZ - minZ + 1);
        int lx = maxX - minX + 1;
        int ly = maxY - minY + 1;
        int lz = maxZ - minZ + 1;
                                         ...
                                         VectorVec5d &resultCurve, Mat2i &resultCurConInfo)
{
    ...
    if ( 0 == indexImg((int)initialPoint(0), (int)initialPoint(1), (int)initialPoint(2)) ){
        ...
        roundInitPt = Vec3d( NGUtility::Round(initialPoint(0) ), NGUtility::Round(initialPoint(1)), NGUtility::Round(initialPoint(2) )  );
    ...
    if (!NGUtility::IsEqualDouble(initDirection(0), 0.0)
        || !NGUtility::IsEqualDouble(initDirection(1), 0.0)
        || !NGUtility::IsEqualDouble(initDirection(2), 0.0)){
        ...
        if(origImg.XResolution() * origImg.YResolution() * origImg.ZResolution() > 1.5){
            for (VectorVec5d::size_type i = 0; i < resultCurveCopy.size(); ++i)
                three.push_back(std::min(2.0,
                                         double(std::max(1.0, double(NGUtility::Round(resultCurveCopy[i](3) + 0.5) )))) );//
        }else{
            for (VectorVec5d::size_type i = 0; i < resultCurveCopy.size(); ++i)
                three.push_back(std::min(5.0,
                double(std::max(1.0, double(NGUtility::Round(resultCurveCopy[i](3) + 2.5) )))) );
        } 
                                         ...
                                         Vec3d &vec1/*, Vec3d &vec2, Vec3d &vec3*/)
{
    ...
    for (int i = 0; i < 36; ++i){
        for (int j = 0; j < 12; ++j){
            ...
            for (int tt = -8; tt <= 8; ++tt){
                ...
                /*该方向上不同长度的强度*/
                minX = std::max(std::min(NGUtility::Round(aRayNode[0] - 1.0 ), sx - 1), 0);
                maxX = std::min(std::max(NGUtility::Round(aRayNode[0] + 1.0 ), 0), sx - 1);
                minY = std::max(std::min(NGUtility::Round(aRayNode[1] - 1.0 ), sy - 1), 0);
                maxY = std::min(std::max(NGUtility::Round(aRayNode[1] + 1.0 ), 0), sy - 1);
                minZ = std::max(std::min(NGUtility::Round(aRayNode[2] - 1.0 ), sz - 1), 0);
                maxZ = std::min(std::max(NGUtility::Round(aRayNode[2] + 1.0 ), 0), sz - 1);
                tmpDense = 0.0; //akmmv2
                /*求强度*/
    ...
    aRayNode << NGUtility::Round(locCenter(0) ), NGUtility::Round(locCenter(1) ), NGUtility::Round(locCenter(2) ) ;
                                           ...
                                           VectorVec5d &resultSeedCurve, Vec2i &curConInfo)
{
    ...
    while(isNextNodeValid == 1 && i < 3000) {
        ...
        Get3DRegion(xMin, xMax, yMin, yMax, zMin, zMax,
            NGUtility::Round(resultSeedCurve[i](0)), NGUtility::Round(resultSeedCurve[i](1)), NGUtility::Round(resultSeedCurve[i](2) ),
        ...
        int rdrvx = std::min(std::max(NGUtility::Round(resultSeedCurve[i](0) ), 0), nxx - 1);
        //int rdrvy = std::min(std::max((int)(resultSeedCurve[i](1) - 3.0 + 0.5), 0), nyy - 1);
      //2014-4-22
        int rdrvy = std::min(std::max(NGUtility::Round(resultSeedCurve[i](1) ), 0), nyy - 1);
        int rdrvz = std::min(std::max(NGUtility::Round(resultSeedCurve[i](2) ), 0), nzz - 1);
...
//            nextDendDir = sx1;
        ...
        if ( i > 3 ){//&& i < 3000
            ...
            NGUtility::GetPartVectorVec3d(resultSeedCurve, std::max<int>(i - 10, 0), i - 1, tmpCurve);
        ...
        if ( i > 3 ){//matlab is 4
            ...
            if (isNextNodeValid>0){//
                VectorVec3d tmpCurve;
                NGUtility::GetPartVectorVec3d(resultSeedCurve, i - 1, i, tmpCurve);
                ...
                for(size_t k = 0; k < origWet.size(); ++k){
                    origbackWet.push_back(origWet[k] - backWet[k]-//endThrev* //2016-3-8
                                          std::sqrt(backWet[k]));
                }
...
//            }
            ...
            tmprs1.push_back(0);
            for (std::deque<int>::size_type ii = 0; ii < jumpIndexVal.size(); ++ii){
                if ( 0 != jumpIndexVal[ii]  ) tmprs1.push_back(int(ii));
            }
            tmprs1.push_back(int(indexWindowVal.size()));
                                      ...
                                      VectorVec5d &resultCurve, Mat2i &resultCurConInfo)
{
    ...
    if (rawSomaCurve.size() > 4){
        ...
        int rawSomaCurveNodeSum = int(rawSomaCurve.size());
        ...
        for (VectorVec5d::size_type i = 0; i < resultCurveCopy.size(); ++i){
            ...
            //2014-4-20
            three.push_back(std::min<double>(2.0, double(NGUtility::Round(resultCurveCopy[i](3) + 0.5) ) ) );
        }
        int id1(0), id2(0), id3(0);
        int xMin, xMax, yMin, yMax, zMin, zMax;
        ...
        int innerSomaPtNum = int(innerSomaPt.size());
                                               ...
                                               VectorVec5d &resultSomaCurve)
{
    ...
    while(isNextNodeValid == 1 && i < 3000){
        int xMin, xMax, yMin, yMax, zMin, zMax;
        Get3DRegion(xMin, xMax, yMin, yMax, zMin, zMax,
            NGUtility::Round(resultSomaCurve[i](0) ), NGUtility::Round(resultSomaCurve[i](1)), NGUtility::Round(resultSomaCurve[i](2)), xExtractRegion, yExtractRegion, zExtractRegion,
        ...
        Vec3d lefttopAxis(xMin, yMin, zMin);
        //2014-4-22
        int rdrvx = std::min(std::max(NGUtility::Round(resultSomaCurve[i](0) - 1.0 ), 0), nxx - 1);
        int rdrvy = std::min(std::max(NGUtility::Round(resultSomaCurve[i](1) - 1.0 ), 0), nyy - 1);
        int rdrvz = std::min(std::max(NGUtility::Round(resultSomaCurve[i](2) - 1.0 ), 0), nzz - 1);
        double threv = backImg(rdrvx, rdrvy, rdrvz);
    ...
    for (int jj = 0; jj < repeat;++jj){
        for (int i = 1; i <= Theta; ++i){
            for (int j = 1; j <= Phi; ++j){
                ...
                //if (std::abs(rayGradWetSum) > 0.001){
                    if ( sphereRayGradient( (std::min)(NGUtility::Round(currentRayLength + 0.5) -1, (int)raySlice.size() - 1), i - 1, j - 1 ) >  gradThrev ){
                                  ...
                                  VectorVec3d &inflatedArea)
{
    ...
    for (VectorVec3d::size_type i = 0; i < innerSomaPtNum; ++i){
        ...
        Get3DRegion(xMin, xMax, yMin, yMax, zMin, zMax, NGUtility::Round(currentSomaPt(0)), NGUtility::Round(currentSomaPt(1)), NGUtility::Round(currentSomaPt(2)),
        ...
        if (windowMinVal < 1025 && currentSomaPt(0) < nx - 1 && currentSomaPt(1) < ny - 1
            ...
            if (windowValList[NGUtility::Round((double)(xLen * yLen * zLen) / 2.0 -0.5)] < 1025){
                                                ...
                                                VectorVec3d &resultInflatedArea)
{
    ...
    for ( int i = 0; i < nxx; ++i){
        ...
        Get3DRegion(xMin, xMax, yMin, yMax, zMin, zMax, NGUtility::Round(initPoints[i](0) ), NGUtility::Round(initPoints[i](1) ), NGUtility::Round(initPoints[i](2) ),
                    1, 1, 1, 0, nx - 1, 0, ny - 1, 0, nz - 1);
                                         ...
                                         const int threv, VectorVec3d &cxDomainSet, std::vector<int> &cxDomainNumList)
{
    ...
    //Vec3d pt;
    int nxx = int(inflatedPoints.size());
    int locWinWet(0);
    ///----------------begin-----------------
    for (int n = 0; n < nxx; ++n){
        ...
        {
            Get3DRegion(xMin, xMax, yMin, yMax, zMin, zMax, NGUtility::Round(pt(0)), NGUtility::Round(pt(1) )
                        , NGUtility::Round(pt(2) ), 1, 1, 1);
        ...
        if (locWinWet > threv){
            ...
            cxDomainNumList.push_back(int(resultPoint.size()));
            resultPoint.clear();
        }//if
    }//for
}
...
void TraceFilter::CalcConstraintPCA(const VectorVec3d &neighborPtSet, const Vec3d &curSeedNode, const Mat3d &convMat,
                           ...
                           double &P, Mat3d &sigmaH, Vec3d &mdata3)//P=\sum{co}, sigmaH=Hg,mdata3=g
{
    Vec3d g;//g
    Mat3d H;//H就是HH
    g.setZero();
                                         ...
                                         Vec3d &nextCurveNode, Vec3d &nextDenDir,  int &isNextNodeValid)
{
    ...
    if (neighborWetNum > std::max(0.1 * threv, 5.0) && thrdk > std::max(endThrev, std::max(0.3 * threv, 4.5 * std::sqrt(threv)))){
        ...
        CalcConstraintPCA(neighborPtSet, curSeedNode, 0.150 * Eigen::Matrix3d::Identity(), neighborWet, P, sigmaH, kk);//P=\sum{co},sigmaH=Hg, kk=g
        if (P>0){
                                                       ...
                                                       VectorMat2i &resultDendConInfo/*, std::vector<int> &denNums*/)
{
    ...
    for (std::vector<VectorVec5d>::size_type i = 0; i < nxx; ++i){
        ...
        if (currentCurve.size() > 10lu || (currentCurve.size() > 3llu && currentConInfo.array().abs().sum() > 0.0)){
            hasConCurveIdList.push_back(int(i));
        }
    }
...
void TraceFilter::AddCollideConnectNodeV2(const std::vector<VectorVec5d> &dendCurves, VectorMat2i &dendConInfo,
...
void TraceFilter::AddCollideConnectNodeSub(VectorMat2i &dendConInfo, Mat2i& currentConInfo, size_t ij,
    const VectorVec3d &somaList, std::vector<VectorVec5d> &resultDendList)
{
...
void TraceFilter::ConnectNeighborCurveToHead(std::vector<VectorVec5d > &resultDendList, VectorMat2i &dendConInfo, size_t ij, Mat2i &currentConInfo)
{
    const VectorVec5d& currentDend = resultDendList[ij];
    Vec3d firstDendPt;
...
void TraceFilter::ConnectNeighborCurveToTail(std::vector<VectorVec5d > &resultDendList, VectorMat2i &dendConInfo, size_t ij, Mat2i &currentConInfo)
{
    const VectorVec5d& currentDend = resultDendList[ij];
    Vec3d firstDendPt;
    ...
    isShortestExist = int(shortestNodeId.size());
    if (isShortestExist < 1) {
        printf("nima\n");
    }
    if (conNodeDistList[0].second < 2.0) {
                                        ...
                                        const VectorVec3d &somaList, std::vector<VectorVec5d> &resultDendList)
{
    ...
    resultDendList = dendCurves;
    double minDistThrev;
    if (origImgPointer->XResolution() * origImgPointer->YResolution() * origImgPointer->ZResolution() > 1.5) {
        minDistThrev = 10.0;
    }
    else{
        minDistThrev = 17.0;//2*5*sqrt(3)
    }
    for ( std::vector<VectorVec5d>::size_type ij = 0; ij < kk; ++ij){
        Mat2i currentConInfo = dendConInfo[ij];//must copy
        if (currentConInfo(0,0) > 0){
            const VectorVec5d& currentDend = resultDendList[ij];
            Vec3d firstDendPt;
            ...
            for (VectorVec5d::size_type ii = 0; ii < nxx; ++ ii){
                datap2 << conDendCurve[ii](0), conDendCurve[ii](1), conDendCurve[ii](2);
                datap22 = firstDendPt - datap2;
                conNodeDistList.push_back(std::pair<int, double>(int(ii) + 1, datap22.norm()));//id, distance
            }
            std::sort(conNodeDistList.begin(), conNodeDistList.end(), Pair_less());
            //Change:2014-3-21-10-32
            int isShortestExist = (int)nxx;//1 or 0
            std::vector<int> shortestNodeId;
            for (int ii = 0; ii < isShortestExist; ++ii){
                if (conNodeDistList[ii].second > minDistThrev)
                    continue;//break;
                shortestNodeId.push_back(conNodeDistList[ii].first);
            }
            isShortestExist = int(shortestNodeId.size());
            if (isShortestExist == 0){
                NG_ERROR_MESSAGE("How could connect info error ?");
                continue;
            }
...
void TraceFilter::ReconstructSomaShapeForTrace(const VectorVec3d &somaList, const SVolume &origImg, Volume<int> &indexImg, VectorMatXd &allRayLen)
{
    ...
    int ny = origImg.y();
    int nz = origImg.z();
    int somaListLen = int(somaList.size());

    for(int ii = 0; ii < somaListLen; ++ii){
        const Vec3d& initialP = somaList[ii];
        int Idex=std::min<int>(std::max<int>(NGUtility::Round(initialP(0)),0), nx-1);
        int Idey=std::min<int>(std::max<int>(NGUtility::Round(initialP(1)),0), ny-1);
        int Idez=std::min<int>(std::max<int>(NGUtility::Round(initialP(2)),0), nz-1);
        VectorVec3d innerSomaPts;
        if(indexImg(Idex, Idey,Idez) == 0){
            ...
            if (origImg.XResolution() * origImg.YResolution() * origImg.ZResolution() > 1.5) {
                ...
                LOG(INFO) <<"ReconstructSomaShapeQuanRevi";
        ...
        for(size_t i = 0; i < innerSomaPtsNum; ++i){
            ...
            roundCurPt << NGUtility::Round(curPt(0)),NGUtility::Round(curPt(1)),NGUtility::Round(curPt(2));
                                           ...
                                           Vec3d &thickDendInitPt)
{
        ...
        for (int j = 1; j <= Phi; ++j){
            if(rayLength(i,j) > 0.0){
                for(double k = 1.0; k <= 0.5 * rayLength(i,j); ++k){
                    ...
                    xx = NGUtility::Round(xx );
                    yy = NGUtility::Round(yy );
                    zz = NGUtility::Round(zz );
    ...
    double aThrev = backImg(NGUtility::Round(currentSoma(0)),NGUtility::Round(currentSoma(1)),NGUtility::Round(currentSoma(2)) );
                                  ...
                                  Vec2i &resultCollideInfo, int &curID, int& resultConInfo)
{
    ...
    curID = origID;
    int id1 = NGUtility::Round(modifiedSomaCurve[curID](0));
    int id2 = NGUtility::Round(modifiedSomaCurve[curID](1));
    int id3 = NGUtility::Round(modifiedSomaCurve[curID](2));
    int xMin, xMax, yMin, yMax, zMin, zMax;
    ...
    VectorVec3d tmpCurve;
    NGUtility::GetPartVectorVec3d(modifiedSomaCurve, std::max<int>(curID - 6, 0),
                                  curID, tmpCurve);
    Vec3d directions;
    ...
    if(curveCollideFlag > 0.5){
        NGUtility::GetPartVectorVec3d(modifiedSomaCurve,
                                     ...
                                     VectorVec3d &crossCurve, int &isContinueTrace)
{
    ...
    while(isNextNodeValid == 1 && i < 14){
        int xMin, xMax, yMin, yMax, zMin, zMax;
        Get3DRegion(xMin, xMax, yMin, yMax, zMin, zMax,
            NGUtility::Round(tmpDendCurve[i](0) ), NGUtility::Round(tmpDendCurve[i](1)), NGUtility::Round(tmpDendCurve[i](2) ), 10, 10, 7,
            0, nxx - 1, 0, nyy - 1, 0, nzz - 1);
...
//        }
        ...
        int rdrvx = std::min(std::max(NGUtility::Round(tmpDendCurve[i](0) - 1.0 ), 0), nxx - 1);
        //2014-4-22
        //int rdrvy = std::min(std::max(int(tmpDendCurve[i](1) - 3.0 + 0.5), 0), nyy - 1);
        int rdrvy = std::min(std::max(NGUtility::Round(tmpDendCurve[i](1) - 1.0 ), 0), nyy - 1);
        int rdrvz = std::min(std::max(NGUtility::Round(tmpDendCurve[i](2) - 1.0 ), 0), nzz - 1);
        ...
        if(isNextNodeValid > 0){
            VectorVec3d tmpCurve;
            NGUtility::GetPartVectorVec3d(tmpDendCurve, i - 1, i, tmpCurve);
            std::vector<double> origWet,backWet;
            WeighRayValue(tmpCurve, origImg,origWet);
            WeighRayValue(tmpCurve, backImg, backWet);
    ...
    if (i > 0) 
        NGUtility::GetPartVectorVec3d(tmpDendCurve, 0, i, crossCurve);
    if(i > 4){
        ...
        for(int ij = 0; ij <= i; ++ij){
            //<=i
            Vec3i curRoundPt;
            curRoundPt << NGUtility::Round(crossCurve[ij](0)),NGUtility::Round(crossCurve[ij](1)),
                          NGUtility::Round(crossCurve[ij](2));
                                     ...
                                     VectorVec3d &crossCurve, int &isContinueTrace)
{
    ...
    while(isNextNodeValid == 1 && i < 14){
        int xMin, xMax, yMin, yMax, zMin, zMax;
        Get3DRegion(xMin, xMax, yMin, yMax, zMin, zMax,
            NGUtility::Round(tmpDendCurve[i](0) ), NGUtility::Round(tmpDendCurve[i](1)), NGUtility::Round(tmpDendCurve[i](2) ), 15, 15, 10,
            0, nxx - 1, 0, nyy - 1, 0, nzz - 1);
                                      ...
                                      const double threv, const Vec3d &x1, Vec3d &pushedNode)
{
    ...
    size_t ny = origImg.y();
    size_t nz = origImg.z();
    int xMin = (std::max)((int)NGUtility::Round(curveNode(0) - 7.0), 0);
    int xMax = (std::min)((int)NGUtility::Round(curveNode(0) + 7.0), (int)(nx) - 1);
    int yMin = (std::max)((int)NGUtility::Round(curveNode(1) - 7.0), 0);
    int yMax = (std::min)((int)NGUtility::Round(curveNode(1) + 7.0), (int)(ny) - 1);
    int zMin = (std::max)((int)NGUtility::Round(curveNode(2) - 6.0), 0);
    int zMax = (std::min)((int)NGUtility::Round(curveNode(2) + 6.0), (int)(nz) - 1);
                          ...
                          CVolume &eroBinImg, VectorVec3d &eroPtSet)
{
    ...
    for(size_t i = 0; i < binPtSet.size(); ++i){
        ...
        binDomainSum = 0;
        Get3DRegion(xMin, xMax, yMin, yMax, zMin, zMax, NGUtility::Round(it(0)), NGUtility::Round(it(1)), NGUtility::Round(it(2)),
                    1, 1, 1,
                    0, nx-1, 0, ny-1, 0, nz-1);
        ///------------------邻域-----------------------------------
        ...
        if (binDomainSum > int(27.0f*( 1.0f/3.0f + 0.01f * eroIntensity))) {
            eroBinImg( NGUtility::Round(it(0)), NGUtility::Round(it(1)) , NGUtility::Round(it(2))) = 255;
...
void TraceFilter::RayBurstShape(const Vec3d &initSoma, const SVolume &v, VectorVec3d &rayNode, DVolume &smoothRay)
{
    ...
    for (auto k = 0; k < numLineSegment; ++k){
        for (int i = 1; i <= Theta; ++i){
            for (int j = 1; j <= Phi; ++j){
        ...
        for(int i = 1; i < Theta + 1; ++i){
            if(resultRayLength(i, j) > 1.0){
                for(double k = 1.0; k <= 0.3 * resultRayLength(i,j); k += 0.5){
                    ...
                    tmp(0) = (double)(NGUtility::Round(xx));
                    tmp(1) = (double)(NGUtility::Round(yy));
                    tmp(2) = (double)(NGUtility::Round(zz));
